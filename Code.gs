...
function submitForApproval_(data) {
  ...
  // Naveen owns the normal workflow. If an earlier V2 test left the
  // stage in PENDING_APPROVAL, this action cleans that up and locks it.
...
function approveAndLock_(data) {
  // Legacy endpoint retained for compatibility, but only Naveen may lock.
  return submitForApproval_(data);
}
