...
function getArticleStatus_(articleNo) {
  ...
  const result = {
    articleNo: articleNo,
    DEVELOPMENT: buildStageStatus_('DEVELOPMENT', byKey),
    FINISHED: buildStageStatus_('FINISHED', byKey)
  };

  return result;
}

function buildStageStatus_(stage, byKey) {
  const rules = CAPTURE_RULES[stage];
  const items = rules.map(rule => ({
    photoType:rule.type,
    required:rule.required,
    captured:!!byKey[stage + '|' + rule.type],
    fileName:byKey[stage + '|' + rule.type]
      ? byKey[stage + '|' + rule.type].fileName : ''
  }));
...
function savePhoto(data) {
  ...
  const status = getArticleStatus_(data.articleNo);
  if (status[data.stage].locked) {
    throw new Error(data.stage + ' capture is locked.');
  }
...
function lockStage_(data) {
  if (!data.articleNo || !data.stage) throw new Error('Article Number and stage are required.');

  validateArticle_(data.articleNo);

  const status = getArticleStatus_(data.articleNo);
  if (!status[data.stage].complete) {
    throw new Error(
      data.stage + ' cannot be locked. Required captures complete: ' +
      status[data.stage].requiredCaptured + '/' +
      status[data.stage].requiredTotal
    );
  }
